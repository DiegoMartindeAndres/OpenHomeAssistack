...
sudo lxc-attach -n br1 --  /etc/init.d/ssh restart
...
sudo lxc-attach -n br1 -- sudo /usr/bin/sshpass -p 'root' scp  -o StrictHostKeyChecking=no root@10.255.0.1:/config/configuration.yaml /root/
...
sleep 25
sudo lxc-attach -n br1 -- sudo /bin/bash /root/script.sh & sleep 5 ; kill $!
